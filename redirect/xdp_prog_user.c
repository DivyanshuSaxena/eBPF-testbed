...
static int parse_mac(char *str, unsigned char mac[ETH_ALEN]) {
  if (parse_u8(str, &mac[0]) < 0) return -1;
  if (parse_u8(str + 3, &mac[1]) < 0) return -1;
  if (parse_u8(str + 6, &mac[2]) < 0) return -1;
  if (parse_u8(str + 9, &mac[3]) < 0) return -1;
  if (parse_u8(str + 12, &mac[4]) < 0) return -1;
  if (parse_u8(str + 15, &mac[5]) < 0) return -1;
...
static int write_iface_params(int map_fd, unsigned char *src,
  ...
  printf(
      "forward: %02x:%02x:%02x:%02x:%02x:%02x -> "
      "%02x:%02x:%02x:%02x:%02x:%02x\n",
      src[0], src[1], src[2], src[3], src[4], src[5], dest[0], dest[1], dest[2],
...
int main(int argc, char **argv) {
  ...
  if (redirect_map) {
    ...
    for (i = 1; i < 256; ++i) bpf_map_update_elem(map_fd, &i, &i, 0);
  }
